...
func (f *fakeKeysetFetcher) GetSigningKeys(_ context.Context) (client.SigningKeyset, error) {
	i := f.calls
	f.calls++
	if i >= len(f.results) {
		return client.SigningKeyset{}, errors.New("unexpected fetch")
	}
	return f.results[i].keyset, f.results[i].err
}

func TestKeysetSource_BootstrapKeys(t *testing.T) {
...
func TestKeysetSource_RefreshAfterTTL(t *testing.T) {
	now := time.Date(2026, time.February, 25, 10, 0, 0, 0, time.UTC)
	clock := func() time.Time { return now }
	fetcher := &fakeKeysetFetcher{
		results: []fetchResult{
	...
	// First call: no bootstrap, must fetch.
...
func TestKeysetSource_FallbackToCache(t *testing.T) {
	now := time.Date(2026, time.February, 25, 10, 0, 0, 0, time.UTC)
	clock := func() time.Time { return now }
	fetcher := &fakeKeysetFetcher{
		results: []fetchResult{
	...
	// While cache is fresh, bootstrap keys are returned.
	...
	// After TTL expires, fetch is attempted and fails -> error (fail-closed).
