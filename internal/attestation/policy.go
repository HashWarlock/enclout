...
// StaticPolicy checks bundle measurements against fixed allowlists.
// An empty allowlist for a measurement means any value is accepted.
type StaticPolicy struct {
	allowedMRTD  map[string]struct{}
	allowedRTMR3 map[string]struct{}
}

// NewStaticPolicy creates a policy that restricts MRTD and RTMR3 to the
// provided allowlists. Nil or empty slices mean "allow any".
...
// CheckMeasurements verifies that the bundle's measurements satisfy the
// policy. It returns a VerificationError with ReasonMeasurementMismatch on
// failure.
